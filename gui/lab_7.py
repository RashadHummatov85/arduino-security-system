"""
Lab Task 7 — Tag Logger GUI
ADA University, Embedded Systems
Author: Rashad

What it does
------------
- Connects to the Arduino over serial (configurable port + baud).
- Reads lines of the form:
      STATE,WAITING | LOCKED | UNLOCKED
      TAG,<UID_HEX>
      INFO,<msg>
- Maintains a SQLite database of unique tags. Each unique UID gets its own
  auto-incrementing internal ID. Repeat scans increment scan_count and update
  last_seen.
- Displays the database in a sortable table; lets you assign a friendly label
  to each tag and delete entries.
- Shows the live system state and a small log pane.

Run
---
    pip install PyQt6 pyserial
    python tag_logger.py

If PyQt6 install fails on your machine, swap to PyQt5 by replacing
"PyQt6" with "PyQt5" everywhere and `QtCore.Qt.AlignmentFlag.AlignCenter`
with `QtCore.Qt.AlignCenter`, etc. (PyQt6 is consistent with your SDP, so
this file targets it.)
"""
