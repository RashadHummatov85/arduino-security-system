 ...
 *   - IR Receiver Module
 *   - RFID-RC522 (SPI)
 *   - Red LED   (locked / waiting indicator)
 *   - Green LED (unlocked indicator)
 *
 * Pin map (no conflicts — see README):
 *   Keypad rows  : A0, A1, A2, A3   (used as digital I/O)
 *   Keypad cols  : D2, D3, D4, D5
 *   IR receiver  : D6
 *   Red LED      : D7
 *   Green LED    : D8 (also drives IRremote feedback — disabled in build flags)
 *   RFID RST     : D9
 *   RFID SS/SDA  : D10
 *   RFID MOSI    : D11   (hardware SPI, fixed)
 *   RFID MISO    : D12   (hardware SPI, fixed)
 *   RFID SCK     : D13   (hardware SPI, fixed)
 *
 * State machine:
 *   WAITING  -> red blinks ~1 Hz, green off, keypad arms the lock
 *   LOCKED   -> red solid,        green off, IR remote can unlock; RFID DISABLED
 *   UNLOCKED -> red off,          green solid, RFID active; brief double-flash on read
 *
 * Serial protocol (115200, newline-terminated):
 *   ARDUINO -> PC :
 *     STATE,WAITING | STATE,LOCKED | STATE,UNLOCKED
 *     TAG,<UID_HEX>
 *     INFO,<msg>
 */
